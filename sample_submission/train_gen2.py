...
def train_gen2():
    ...
    # Load dataset
    dataset_path = "selfplay_dataset_lopunny_gen2.pt"
    ...
    # Initialize from Gen 1 weights
    gen1_weights = "value_network_lopunny_weighted_best.pth"
    if not os.path.exists(gen1_weights):
        if os.path.exists(os.path.join("sample_submission", gen1_weights)):
            gen1_weights = os.path.join("sample_submission", gen1_weights)
            
    if os.path.exists(gen1_weights):
        print(f"Initialisation du modèle avec les poids Gen 1: {gen1_weights}")
        model.load_state_dict(torch.load(gen1_weights, map_location=device))
    else:
        print(f"Warning: Poids Gen 1 non trouvés à {gen1_weights}. Initialisation aléatoire.")
        
    # Fine-tuning: Lower learning rate (2e-4) to prevent catastrophic forgetting
    optimizer = optim.Adam(model.parameters(), lr=2e-4, weight_decay=1e-4)
    criterion = nn.MSELoss(reduction='none')
    
    epochs = 15
    print(f"\nLancement du Fine-Tuning Gen 2 (epochs={epochs}, lr=2e-4)...")
    ...
    best_model_path = os.path.join(out_dir, "value_network_lopunny_gen2_best.pth")
    final_model_path = os.path.join(out_dir, "value_network_lopunny_gen2.pth")
    ...
    try:
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss (MSE)')
        plt.title('Lopunny Gen 2 Fine-Tuning Loss Curves')
        plt.legend()
        plt.grid(True)
        plot_name = "loss_curves_lopunny_gen2.png"
    ...
    try:
        ...
        metrics_path = os.path.join(out_dir, "training_metrics_lopunny_gen2.json")
